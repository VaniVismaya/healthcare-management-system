...
export default function RoleSelect() {
  ...
  const roles = [
    { id: 'patient', icon: User, label: t('roleSelect.patient'), desc: t('roleSelect.patientDesc'), color: '#0A7EA4', bg: '#E8F4F8' },
    { id: 'doctor', icon: Stethoscope, label: t('roleSelect.doctor'), desc: t('roleSelect.doctorDesc'), color: '#00B894', bg: '#E8F8F4' },
    { id: 'laboratory', icon: FlaskConical, label: t('roleSelect.laboratory'), desc: t('roleSelect.labDesc'), color: '#7C3AED', bg: '#F3EDFE' },
    { id: 'pharmacist', icon: Pill, label: t('roleSelect.pharmacist'), desc: t('roleSelect.pharmacistDesc'), color: '#F59E0B', bg: '#FEF7E8' },
  ];
      ...
      <div style={{ width: '100%', maxWidth: 600 }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(20px)', borderRadius: 24, padding: 32, border: '1px solid rgba(255,255,255,0.15)' }}>
          {user && (
            <div style={{ background: 'rgba(255,255,255,0.85)', borderRadius: 12, padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 16 }}>
              <div style={{ fontSize: 12, color: '#0D2137', fontWeight: 600 }}>
                You are logged in as {user.role}. Continue to your dashboard or choose a role to switch.
              </div>
          ...
          <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 13, textAlign: 'center', marginBottom: 28 }}>
            Login is common for everyone. Choose a role only when creating a new account.
          </p>
